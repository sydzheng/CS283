...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        last_rc = ((char)io_buff[rc-1] == RDSH_EOF_CHAR) ? 1 : 0;
        ...
        if (cmd_rc == ERR_CMD_OR_ARGS_TOO_BIG) {
            send_message_string(cli_socket, "error: argc 9 max\n");
            continue;
        }  
        ...
        if (cmd_rc == ERR_TOO_MANY_COMMANDS) {
            send_message_string(cli_socket, "error: pipe limit reached\n");
            continue;
        }
        ...
        if (bi_cmd == BI_CMD_CD) {
            if (send_message_string(cli_socket, io_buff) == ERR_RDSH_COMMUNICATION) {
                free(io_buff);
                return ERR_RDSH_COMMUNICATION;
            }
            continue;
...
int send_message_string(int cli_socket, char *buff){
    ...
    eof_len = send_message_eof(cli_socket);

    if (eof_len == ERR_RDSH_COMMUNICATION) {
        printf(CMD_ERR_RDSH_COMM);
        return ERR_RDSH_COMMUNICATION;
    }
